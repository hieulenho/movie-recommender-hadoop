...
| Milestone 3: Maven and Hadoop environment | In progress: add Java build configuration and local Hadoop execution plan. | Maven project files, smoke MapReduce job, Java tests, run scripts, and environment notes. | Complete only after Java 17/Maven validation succeeds, the local smoke job writes `lineCount<TAB>5`, and generated files remain untracked. |
| Milestone 4: User-history MapReduce job | Next: group ratings by user. | User-history output records. | Job output matches documented format and reference expectations on fixtures. |
