...
input_path="${1:-$repo_root/tests/fixtures/user-history/ratings.csv}"
output_path="${2:-$repo_root/target/user-history-output}"
...
if [[ ! -f "$input_path" ]]; then
  echo "User-history input file does not exist: $input_path" >&2
  exit 1
fi
...
classpath_file="$repo_root/target/user-history-classpath.txt"
