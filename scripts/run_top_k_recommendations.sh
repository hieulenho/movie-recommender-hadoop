...
user_history_input="${1:-$repo_root/tests/fixtures/top-k-recommendation/user-history.txt}"
raw_prediction_input="${2:-$repo_root/tests/fixtures/top-k-recommendation/raw-predictions.txt}"
output_path="${3:-$repo_root/target/top-k-recommendation-output}"
top_k="${4:-2}"
...
case "$top_k" in
  ''|*[!0-9]*)
    echo "top-k must be a positive integer: $top_k" >&2
    exit 1
    ;;
esac
if (( top_k < 1 )); then
  echo "top-k must be at least 1." >&2
  exit 1
fi

user_history_input="$(resolve_input_file "$user_history_input" "User-history input")"
raw_prediction_input="$(resolve_input_file "$raw_prediction_input" "Raw-prediction input")"
...
for input_path in "$user_history_input" "$raw_prediction_input"; do
...
classpath_file="$repo_root/target/top-k-recommendations-classpath.txt"
